...
2016年分の過去データ追加(2026-07-24)。既存最古日(2017-07-05)の直前から
さらに約1年分さかのぼり、2016-07-05〜2017-07-04(365日)を取り込む。
2016年は閏年だが閏日(2016-02-29)は今回の範囲外(2016年後半のみ)のため、
365日固定で既存最古日と隙間なく連続する。

このバックフィルで10年分の過去データ拡張シリーズが完了する見込み
(2017〜2026年の追加が既に完了しており、2016年分でちょうど10年)。

collect_results.py と全く同じ取得・解凍・パース・レコード形式
(collect_results.append_from_txt() をそのまま再利用)で、results/{年}.jsonl に
追記する(日付に応じて2016.jsonl/2017.jsonlへ自動振り分け)。新規レコードのみ
追加、既存データの変更・削除はしない。

書式は事前調査で2016年ファイルが現行パーサーで完全動作することを確認済み。
...
START = datetime.date(2016, 7, 5)
END = datetime.date(2017, 7, 4)
POLITE_WAIT = 3          # 秒(公式サーバーへの配慮)
SEVENZIP = r"C:\Program Files\7-Zip\7z.exe"
MAX_CONSECUTIVE_FAILURES = 10   # 2016年は特殊事情なしの見込みのため通常水準
MAX_CONSECUTIVE_EMPTY = 3       # 解凍成功したのに0レースが連続したら書式違いの疑いで停止

LOG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "backfill_2016_log.txt")

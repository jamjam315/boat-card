...
2019年分の過去データ追加(2026-07-22)。既存最古日(2020-07-05)の直前から
さらに約1年分さかのぼり、2019-07-05〜2020-07-04を取り込む。

【期間が366日である理由】2020年は閏年(2020-02-29を含む)。単純に365日固定で
逆算すると2019-07-06〜2020-07-04になり、既存最古日(2020-07-05)との間に
2019-07-05の1日分の隙間ができてしまう。既存データと隙間なく連続させることを
優先し、この回だけ366日(2019-07-05〜2020-07-04)とする。

collect_results.py と全く同じ取得・解凍・パース・レコード形式
(collect_results.append_from_txt() をそのまま再利用)で、results/{年}.jsonl に
追記する(日付に応じて2019.jsonl/2020.jsonlへ自動振り分け)。新規レコードのみ
追加、既存データの変更・削除はしない。

【この範囲後半(2020年前半)の注意】新型コロナ初期にあたり、開催中止・延期が
最も多かった可能性のある時期。欠落日があっても「その日は非開催」であれば
異常ではない。極端な連続404(数週間規模)があれば停止する(閾値は2020年
バックフィルと同じく緩め: 20日連続)。
...
START = datetime.date(2019, 7, 5)
END = datetime.date(2020, 7, 4)
POLITE_WAIT = 3          # 秒(公式サーバーへの配慮)
SEVENZIP = r"C:\Program Files\7-Zip\7z.exe"
MAX_CONSECUTIVE_FAILURES = 20   # 範囲後半がコロナ初期にあたるため他年より緩め(数週間規模のみ異常とみなす)
MAX_CONSECUTIVE_EMPTY = 3       # 解凍成功したのに0レースが連続したら書式違いの疑いで停止

LOG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "backfill_2019_log.txt")

...
【ジョブが緑のままのとき】--what
continue-on-error の手順と自前の関門(QUIZ_OK など)で作ると、中が止まってもジョブは緑で終わる。
その場合 run の jobs を見ても失敗は出てこないので、何が止まったかを呼ぶ側が --what で渡す。
今日の一問の出題がこれで6晩配られず、誰も気づかなかった(2026-09-18〜24)。
...
【手元での確かめ方】
  ...
  python scripts/notify_run_failure.py --title "テスト" --dry --what "今日の一問の作り置き"
...
def body_of(env, parts, hint, soft=False):
    """soft: ジョブは緑で、中の関門だけが閉じたとき(--what)。「失敗しました」とは書かない。"""
    ...
    lines = [
        ...
        % (now.strftime("%Y-%m-%d %H:%M"), env.get("GITHUB_WORKFLOW", "?"), env.get("GITHUB_EVENT_NAME", "?"),
           "で、止まったところがあります(run は緑のまま終わります)。" if soft else "が失敗しました。"),
        "",
        "止まったところ: " + (", ".join(parts) or "(ジョブの一覧を読めませんでした)"),
        "run: " + url,
    ]
    if hint:
        lines += ["", hint]
...
def main():
    ...
    ap.add_argument("--what", default="", help="止まったところを呼ぶ側が書く(ジョブは緑で、中の関門だけが閉じたとき)")
    ...
    if a.what:
        # ジョブは緑なので run の jobs には何も出てこない。呼ぶ側の言葉をそのまま使う。
        parts = [a.what]
    else:
        if a.fake_jobs:
    ...
    print("[run-failure] 止まったところ: %s" % (", ".join(parts) or "不明"))
    marker = "<!-- run-failure:%s-%s -->" % (env.get("GITHUB_RUN_ID", "?"), env.get("GITHUB_RUN_ATTEMPT", "1"))
    ok = gh_issue.notify(a.title, body_of(env, parts, a.hint, soft=bool(a.what)), marker,
                         dry=a.dry, tag="[run-failure]")

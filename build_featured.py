...
if __name__ == "__main__":
    ...
    #
    # ただし黙って正常終了すると、CIが緑のままで失敗に気づけない。実際
    # 2026-07-13〜08-26の45日間、ログの1行を読まない限り分からなかった。
    # GitHub Actionsの注釈(::warning::)として出し、実行ページの上部に必ず残す。
    # 終了コードは0のまま(=ここで止めない方針)を維持する。
    try:
        main()
    except Exception as e:
        # 改行が混じると注釈が途中で切れるので、1行に潰してから渡す。
        detail = str(e).replace("\r", " ").replace("\n", " ")
        print(f"::warning::注目選手の選出に失敗しました。featured.jsは更新されず前回ぶんが残ります: {detail}")

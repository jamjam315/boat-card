...
2021年分の過去データ追加(2026-07-21)。既存最古日(2022-07-05)の直前から
さらに約1年分さかのぼり、2021-07-05〜2022-07-04(365日)を取り込む。

collect_results.py と全く同じ取得・解凍・パース・レコード形式
(collect_results.append_from_txt() をそのまま再利用)で、results/{年}.jsonl に
追記する(日付に応じて2021.jsonl/2022.jsonlへ自動振り分け)。新規レコードのみ
追加、既存データの変更・削除はしない。
...
START = datetime.date(2021, 7, 5)
END = datetime.date(2022, 7, 4)
POLITE_WAIT = 3          # 秒(公式サーバーへの配慮)
SEVENZIP = r"C:\Program Files\7-Zip\7z.exe"
MAX_CONSECUTIVE_FAILURES = 10   # ダウンロード失敗が連続でこの回数を超えたら異常とみなし停止
MAX_CONSECUTIVE_EMPTY = 3       # 解凍成功したのに0レースが連続したら書式違いの疑いで停止

LOG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "backfill_2021_log.txt")

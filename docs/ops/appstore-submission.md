...
| 審査用スクリーンショット | `<アプリの購入画面のスクショ>`（「¥480 / 月で登録する」が写っているもの。手順6の購入画面のスクショを流用してよい） |
...
| バージョン | スクリーンショット（ログインが要る2枚） | 購入画面・マイページ。ログイン不要の4枚は teiyomi-ios `store/app-store/6.9/` |
| バージョン | バージョンのリリース | 手動／承認後に自動 |
| バージョン | ビルド | TestFlight に上がった 1.0 (2) を選ぶ |
| サブスクリプション | 審査用スクリーンショット | 購入画面（上の1枚を流用可） |

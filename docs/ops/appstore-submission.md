...
初回（上の 1〜8）と違い、**変えるところだけ**でよい。App 情報・価格・プライバシー・
サブスクリプションは前のままでよく、審査メモも残っている。

1. ビルドを上げる（版と番号は `pubspec.yaml` に入れてある。`1.0.1+3`）

```bash
cd ~/dev/teiyomi-ios && git pull && flutter build ipa --release
```

```bash
open ~/dev/teiyomi-ios/build/ios/archive/Runner.xcarchive
```

Organizer →「Distribute App」→「App Store Connect」→「Upload」。

2. ASC で「＋ バージョンを追加」→ `1.0.1`
3. **このバージョンの新機能**（利用者に見える文章）

```
・アプリを開くのが少し軽くなりました（見ているタブだけ読み込むようにしました）
・お支払いの確認まわりの不具合を直しました（支払いの猶予期間中にプレミアムが外れることがある問題）
```

4. ビルドに `1.0.1 (3)` を選ぶ
5. スクリーンショットを差し替えるなら、このタイミング（バージョンの提出とセットでしか変えられない）
6. 「審査へ提出」。審査メモ・審査用アカウントは前のものが残っているので、そのままでよい

**サーバ側の修正（猶予期間・TestFlight の購入）は 1.0.1 を待たずに反映済み。**
アプリの入れ替えが要るのは「開いたタブだけ読み込む」だけ。

## 公開後のバックログ（2026-09-17 公開・優先順）

アプリの入れ替え（1.0.1）が要るものと、**サーバだけで直せるもの**を分けてある。
サーバだけのものは、審査を待たずにいつでも出せる。

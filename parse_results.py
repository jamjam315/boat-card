...
#
# 【着順欄はコードのことがある(2026-07-30)】
# 完走した艇は "01"〜"06" だが、完走しなかった艇は公式が記号を入れる。
# 生K票の実測で確認したもの: F(フライング) / L0(出遅れ) / K0(不完走) /
# S0・S1・S2(失格) / まれに "00"。行の並びは完走艇とまったく同じで、
# レースタイムの欄だけが " . . " になる。
# 以前はここを (\d{2}) に限っていたため、この行が結果として取れず
# 「6艇そろわないレース」ができ、それを丸ごと捨てていた(全体の約7%)。
# 記号の意味は公式の定義に踏み込まず、拾った文字列をそのまま残す方針にした。
RESULT = re.compile(
    r'^\s*(\d{2}|[A-Z]\d?)\s+'   # 着順(数字 or 非完走コード)
    r'([1-6])\s+'          # 艇番
    r'(\d{4})\s+'          # 登番
    r'(.+?)\s+'            # 選手名
    r'(\d+)\s+'            # モーター番号
...
def parse_results(path):
    ...
    for line in lines:
        ...
        if mres and cur is not None:
            ...
            # 着順欄が数字(1〜6)なら着順、そうでなければ非完走。
            # 非完走は着順を持たせず(None)、公式の記号をそのまま「着状態」に残す。
            chaku_v = int(chaku) if chaku.isdigit() and 1 <= int(chaku) <= 6 else None
            cur["結果"].append({
                "着順": chaku_v,
                "着状態": None if chaku_v is not None else chaku,
                "艇番": int(tei), "登番": touban,
    ...
    # 【2026-07-30】以前はここで「6艇そろったレースだけ返す」としていたが、
    # 1艇でも失格・転覆・落水があると6艇そろわないため、実在するレースを
    # 丸ごと捨てていた(生K票との突き合わせで全体の約7%)。決まり手「恵まれ」は
    # 定義上かならず6艇そろわないので、100%失われていた。
...
def boat_record(x):
    """results/{年}.jsonl に書き出す艇1件ぶんを作る。

    書き出す側(collect_results.py と rebuild_results.py)で同じ形を2か所に
    書くと片方だけ直して食い違うため、ここに集約している。

    完走艇の中身・キーの並びは従来と一切変えていない(既存の消費側と互換)。
    非完走艇だけ 着=None になり、公式の着順欄の記号(F/L0/K0/S0/S1/S2等)が
    「状」に入る。完走艇に「状」は付けない(ファイルを無駄に太らせないため)。
    """
    rec = {"着": x["着順"], "艇": x["艇番"], "進": x["進入コース"],

 ...
 *
 * 講評そのものは日本語300〜500字ぶんで、実測の本文は418〜515字(≒350〜450
 * トークン)だった。本文だけなら800でも足りる。大きく取ってあるのは
 * **GPT-5世代が推論トークンを同じ予算から食う**ため。
 *
 * レジャー帳は1〜3文の発話で1024にして、本文が空になるのは1%前後だった。
 * 艇読みの講評は7〜10行で、出力がその3〜5倍ある。**1024で本番実測したら
 * 8回中2回(約25%)が本文空**になった(2026-09-03)。同じ数字を持ってきても、
 * 頼んでいる文量が違えば足りない。
 *
 * 2048にすると、観測した本文(450トークン)の外に1600トークンぶんの推論の
 * 余地が残る。天井であって指定ではないので、短く済んだ回の請求は増えない。
 */

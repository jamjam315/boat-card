...
2022年分の過去データ追加(2026-07-20)。既存最古日(2023-07-05)の直前から
さらに約1年分さかのぼり、2022-07-05〜2023-07-04(365日)を取り込む。

collect_results.py と全く同じ取得・解凍・パース・レコード形式
(collect_results.append_from_txt() をそのまま再利用)で、results/{年}.jsonl に
追記する(日付に応じて2022.jsonl/2023.jsonlへ自動振り分け)。新規レコードのみ
追加、既存データの変更・削除はしない。
...
LOG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "backfill_2022_log.txt")

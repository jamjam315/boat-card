...
既にresults.jsonlへ入っている過去分にはこの項目が無い。results.jsonlに実在する
全日付ぶんのKファイルを取り直し、既存レコード(date:会場:レース番号が一致するもの)に
払戻だけ追記する。着順・進入・ST・展示タイム等の既存項目は一切変えない。
新規レコードは追加しない(取得済みの日付だけを対象にするため)。

生データ(ダウンロードしたlzh/txt)はこのPC上だけで処理し、GitHubには出力の
results.jsonl(要約側の1行1レース形式)だけを反映する。従来方針(backfill_motor.py)を踏襲。
...
def main():
    ...
    print(f"[info] 対象日数: {len(dates)}日 / 対象レコード数: {len(order)}")
    ...
    print(f"[done] {len(dates)}日ぶん処理、{updated_races}/{len(order)}レースに払戻を追記")
    if missing_days:
        shown = missing_days[:10]
        more = "..." if len(missing_days) > 10 else ""
        print(f"[warn] 取得できなかった日: {len(missing_days)}日 {shown}{more}")

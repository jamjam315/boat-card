...
2018年分の過去データ追加(2026-07-23)。既存最古日(2019-07-05)の直前から
さらに約1年分さかのぼり、2018-07-05〜2019-07-04(365日)を取り込む。
2019年は平年(閏年ではない)のため、365日固定でも既存最古日と隙間なく連続する。

collect_results.py と全く同じ取得・解凍・パース・レコード形式
(collect_results.append_from_txt() をそのまま再利用)で、results/{年}.jsonl に
追記する(日付に応じて2018.jsonl/2019.jsonlへ自動振り分け)。新規レコードのみ
追加、既存データの変更・削除はしない。

2018年は特殊事情のない通常年の見込み(2020年のコロナ影響のような懸念材料は
特に無い)。書式違い等が見つかった場合のみ停止する。
...
START = datetime.date(2018, 7, 5)
END = datetime.date(2019, 7, 4)
POLITE_WAIT = 3          # 秒(公式サーバーへの配慮)
SEVENZIP = r"C:\Program Files\7-Zip\7z.exe"
MAX_CONSECUTIVE_FAILURES = 10   # 2018年は特殊事情なしの見込みのため通常水準
MAX_CONSECUTIVE_EMPTY = 3       # 解凍成功したのに0レースが連続したら書式違いの疑いで停止

LOG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "backfill_2018_log.txt")
